function [x_out, output] = genHyBR_land(A, b, Q, R,land,options)
%
...
if ~notrue
  nrmtrue = norm(x_true(:));
  x_trueland = x_true(land);
  nrmtrueland = norm(x_trueland(:));
end
...
switch solver
  ...
  case 'tikhonov'
    solverhandle = @Tikhonovsolver_land;
end
  ...
  if i >= 2 %Begin GK iterations
    ...
    switch insolve
      case {'tsvd', 'tikhonov'}
        ...
        [f, alpha] = feval(solverhandle, Ub, diag(Sb), Vb, vector, options, B, beta, QV, m, land);
...
%     x = Q*(V*f);
    ...
    if outputparams
      if ~notrue
        ...
        xland = x(land); 
        output.Enrm(i,1) = norm(xland(:)-x_trueland(:))/nrmtrueland;
      end
