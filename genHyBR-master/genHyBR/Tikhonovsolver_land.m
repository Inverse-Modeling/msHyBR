function [x, alpha] = Tikhonovsolver_land(U, S, V, b, options, B, beta, QV, m, land)
%
%         [x, alpha] = Tikhonovsolver(U, S, V, b, options, QV, m)
%
...
%       QV - covariance matrix (for gen-HyBR)*Golub-Kahan basis vectors
...
switch alpha
  ...
  case 'optimal'
    ...
    if isfield(options,'mu')
      mu = options.mu;
      errhan = @(lambda)TikOPT_land(lambda, V, bhat, S, QV,x_true-mu, land);
    else
      errhan = @(lambda)TikOPT_land(lambda, V, bhat, S, QV,x_true, land);
    end
