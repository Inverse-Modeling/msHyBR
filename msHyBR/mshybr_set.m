function options = sshybr_set(varargin)
%
%   OPTIONS = sshybr_set(varargin)
%
%   Create/alter options structure for sshybr code.
%   OPTIONS = sshybr_set('PARAM1',VALUE1,'PARAM2',VALUE2,...) 
...
%     parameter when passed to the sshybr function). It is sufficient to type
...
%
%   OPTIONS = sshybr_set(OLDOPTS,'PARAM1',VALUE1,...) creates a copy of
%     OLDOPTS with the named parameters altered with the specified values.
%
%   OPTIONS = sshybr_set(OLDOPTS,NEWOPTS) combines an existing options structure
...
%     OLDOPTS.
%
%   sshybr_set with no input arguments and no output arguments displays all
%     parameter names and their possible values, with defaults shown in {}
%
%   OPTIONS = sshybr_set(with no input arguments) creates an options structure
%     where all the fields are set to [].
%
%   OPTIONS = sshybr_set('sshybr') creates an options structure with all
%     the parameter names and default values relevant to 'sshybr'. That is,
%           sshybr_set('sshybr')
%   or
%           sshybr_set(@sshybr)
%   returns an options structure containing all the parameter names and
%   default values relevant to the function 'sshybr'.
%
% sshybr_set PARAMETERS for MATLAB: ( default parameter in {} )
...
%       OPTIONS = sshybr_set('RegPar','WGCV', 'Omega',.5);
%     To change the maximum iterations to 150 in OPTIONS
%       OPTIONS = sshybr_set(OPTIONS,'Iter',150);
%
%   J.Chung 12/2021
%   Modified by M. Sabate Landman 12/2023
%
...
if (nargin == 0) && (nargout == 0)
    ...
    fprintf('               thr: [  non-negative scalar | {10^-3}  ]\n');
    fprintf('              mask: [ array | {off} ]\n');
    return;
end
...
switch field
    ...
    case {'thr'}% real non-negative scalar
        [validvalue, errmsg] = nonNegscalar(field,value);
    case {'mask'} % numeric array, off
        [validvalue, errmsg] = x_truetype(field,value);
  otherwise
        validfield = false;  
        validvalue = false;
...
valid =  ischar(value) && any(strcmp(value,{'tsvd','tikhonov','none','upre_gen2','wgcv_gen2','new_wgcv_gen2','gcv_gen2','wgcv-sgen'}));
if ~valid
